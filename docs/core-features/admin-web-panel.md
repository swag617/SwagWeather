...
* **Live State**: current intensity, current season, days remaining in that season, and the queued forecast for every managed world (same data `/sweather status` and `/sweather forecast` expose).
* **Settings**: world allow/deny lists, the weather and season master switches, forecast tuning (check interval, forecast size, min/max transition minutes), the per-intensity weight sliders, and season length mode/value/order. Saving here writes straight into `config.yml` and reloads live; this has the same effect as running `/sweather reload` after hand-editing the file.
